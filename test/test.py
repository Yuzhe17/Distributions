...
from distributions import Poison
...
class TestPoisonClass(unittest.TestCase):
    def setUp(self):
        self.poison = Poison(5,3)
    
    def test_initialization(self):
        self.assertEqual(self.poison.interval,5,'interval value is not correct')
        self.assertEqual(self.poison.events_per_time,3,'events_per_time not correct')
    
    def test_calculatepmf(self):
        self.assertEqual(round(self.poison.calculate_pmf(4),5),0.00065)
        self.assertEqual(round(self.poison.calculate_pmf(12),5),0.08286)
        self.assertEqual(round(self.poison.calculate_pmf(15),5),0.10244)

    def test_plotbarpmf(self):
        x=list(range(31))
        y=list(map(self.poison.calculate_pmf,x))
        self.assertEqual(self.poison.plot_bar_pmf(),(x,y))
    
    def test_waitingtime(self):
        self.assertEqual(round(self.poison.calculate_waiting_time(3),5),0.00012)
        self.assertEqual(round(self.poison.calculate_waiting_time(2),5),0.00248)
